...
def run_dynamic_analysis(package_name, duration=60, runs=1, output_dir=None):
    ...
    try:
        ...
        # pipeline_runner.js가 이미 토큰화했는지 확인
        if os.path.exists(tokenized_file):
            safe_print(f"[+] 토큰화 파일 발견: {tokenized_file}")
            shutil.copy(tokenized_file, token_path)
            safe_print(f"[+] ✅ 토큰화 완료 (기존 사용): {token_path}")
        elif tokenizer_script.exists():
            # 토큰화 실행 (source_file 사용)
            cmd_tokenize = (
        ...
        # ===== 4. 중복 제거 (cleanup_dynamic_tokens.py) =====
