...
def run_static_analysis(apk_path, output_dir=None):
    ...
    try:
        ...
        cmd1 = (
            ...
            f'--apk "{abs_apk_path}" '
            f'--sources "method/sources_merged.txt" '
            f'--sinks "method/sinks_merged.txt" '
            f'--dyn-methods "method/dyn_methods_merged.txt" '
            f'--out "{taint_out}" '
            f'--full-trace'
        )
