...
# def find_static_csvs_in_case(case_dir: str):
#     case_path = Path(case_dir)
#     export_dir = case_path / "Export"
#     if not export_dir.exists():
#         return []

#     # static_*.csv 찾기
#     csvs = sorted(export_dir.glob("static_*.csv"))
...
#     # 결과 저장
#     base_name = os.path.basename(merged_csv_path).replace('merged_', 'scored_').replace('static_', 'scored_static_')
...
#     if len(sys.argv) < 2:
#         print("사용법: python scoring_runner.py <Case폴더경로> [범죄유형] [출력디렉토리]")
#         print("예: python scoring_runner.py D:\\Cases\\Case_001 살인")
...
#     static_csvs = find_static_csvs_in_case(case_dir)
#     if not static_csvs:
#         print(f"[FAIL] Export 폴더에서 static_*.csv를 찾지 못했습니다: {case_dir}")
#         sys.exit(1)

#     print(f"[+] 발견된 static CSV: {len(static_csvs)}개")
#     for p in static_csvs:
#         print(f"   - {p}")

#     ok_count = 0
#     for csv_path in static_csvs:
#         result = run_scoring(csv_path, crime, out_dir)
#         if result:
#             ok_count += 1
#             print(f"[OK] 성공: {result}")
#         else:
#             print(f"[FAIL] 실패: {csv_path}")
...
"""
scoring_runner.py
새 스코어링 시스템(priority_scoring_system_2.py) 적용
"""
import sys, os
import pandas as pd
from pathlib import Path
...
def run_scoring(merged_csv_path, crime_type='살인', output_dir=None):
    """
    merged CSV 파일에 새 스코어링 시스템 적용
    
    출력: category, path, score (3컬럼)
    """
    print("=" * 60)
    print(f"=== 우선순위 스코어링 v2.0 시작: {crime_type} ===")
    print("=" * 60)
    ...
    paths = [
        str(row[path_column])
        for _, row in df.iterrows()
        if pd.notna(row[path_column]) and str(row[path_column]).strip()
    ]
    
    print(f"[+] {len(paths)}개 유효 경로 확인")
    ...
    # 결과 저장 (새 형식: category, path, score)
    base_name = os.path.basename(merged_csv_path).replace('static_', 'scored_static_').replace('merged_', 'scored_')
    
    if output_dir:
        output_path = os.path.join(output_dir, base_name)
    else:
        output_path = os.path.join(os.path.dirname(merged_csv_path), base_name)
    
    try:
        scorer.export_csv(results, output_path)
        print(f"[+] 저장 완료: {output_path}")
    except Exception as e:
        print(f"[ERROR] 파일 저장 실패: {e}")
        return None
    ...
    print("\n=== Top 5 우선순위 ===")
    for i, r in enumerate(results[:5], 1):
        print(f"{i}. {r.final_score:.1f}점 - {r.category}")
        print(f"   {r.path}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("사용법: python scoring_runner.py <Case폴더경로> [범죄유형] [출력디렉토리]")
        print("예: python scoring_runner.py D:\\Cases\\Case_001 살인")
        print("범죄유형: 절도, 강도, 폭력, 살인, 강간/추행, 사기 (기본값: 살인)")
        sys.exit(1)
